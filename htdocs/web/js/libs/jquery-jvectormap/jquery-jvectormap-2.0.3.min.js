...
jvm.$ = jQuery, Array.prototype.indexOf || (Array.prototype.indexOf = function (searchElement, fromIndex) {
    ...
    var case_usa = ($(this.node).attr('data-code') === 'US' && $(this.node).parent().parent().parent().parent().parent().parent().parent().hasClass('with-usa'));
    if (case_usa) {
        var $map_lister_parent = $(this.node).parent().parent().parent().parent().parent().parent().parent().parent().parent();
        var selected_data = $map_lister_parent.mapLister('getData');
        if (this.isHovered) {
            $(this.node).attr('fill', '#fcb400');
        } else if (collection_contain_usa_markets(selected_data)) {
            $(this.node).attr('fill', '#82BBD6');
        } else {
            $(this.node).attr('fill', this.style.initial.fill);
        }
    }
    ...
    this.isSelected !== isSelected && (this.isSelected = isSelected, this.shape && (this.shape.isSelected = isSelected, this.shape.updateStyle()), this.label && (this.label.isSelected = isSelected, this.label.updateStyle()), this.shape && (jvm.$(this.shape).trigger("selected", [isSelected])));
    var code = this.config.code;
    var case_under_fake_1 = (code === 'DF3');
    var case_under_fake_2 = (code === 'DF1');
    var case_under_fake_3 = (code === 'DF4');
    var case_under_fake_4 = (code === 'DF2');
    var special_case = (case_under_fake_1 || case_under_fake_2 || case_under_fake_3 || case_under_fake_4);
    if (special_case) {
        var fakeBackgroundColor = (isSelected) ? this.map.params.backgroundColor : "#4F6E99";
        if (case_under_fake_1) {
            jQuery('.under_fake_1').attr('fill', fakeBackgroundColor);
        } else if (case_under_fake_2) {
            jQuery('.under_fake_2').attr('fill', fakeBackgroundColor);
        } else if (case_under_fake_3) {
            jQuery('.under_fake_3').attr('fill', fakeBackgroundColor);
        } else if (case_under_fake_4) {
            jQuery('.under_fake_4').attr('fill', fakeBackgroundColor);
        }
    }
    ...
    this.config = config, this.map = this.config.map;
    if (config.code === 'fake_1' || config.code === "fake_2" || config.code === 'under_fake_1' || config.code === 'under_fake_2' || config.code === 'under_fake_3' || config.code === 'under_fake_4') {
        if ($('.full-content-explore-list').length > 0) {
            config.style.initial.fill = (config.code === 'fake_1' || config.code === "fake_2") ? "#44779a" : "#172E62";
        } else {
            config.style.initial.fill = "#4F6E99";
        }
    }
    if (config.path) {
        ...
        if (config.code === 'US' && $(this.map.container).parent().parent().parent().parent().hasClass('with-usa')) {
            var $map_lister_parent = $(this.map.container).parent().parent().parent().parent().parent().parent();
            var selected_data = $map_lister_parent.mapLister('getData');
            console.log('case_usa_debut', collection_contain_usa_markets(selected_data));
            console.log('case_usa_debut', selected_data);
            if (collection_contain_usa_markets(selected_data)) {
                $(this.shape.node).attr('fill', '#82BBD6');
            } else {
                $(this.shape.node).attr('fill', this.config.style.initial.fill);
            }
        }
        if (
            config.code !== 'fake_1' &&
            config.code !== "fake_2" &&
            config.code !== 'under_fake_1' &&
            config.code !== 'under_fake_2' &&
            config.code !== 'under_fake_3' &&
            config.code !== 'under_fake_4'
        ) {
            this.shape.addClass("jvectormap-region jvectormap-element");
        } else {
            this.shape.addClass(config.code);
        }
        bbox = this.shape.getBBox(), text = this.getLabelText(config.code), this.config.label && text && (offsets = this.getLabelOffsets(config.code), this.labelX = bbox.x + bbox.width / 2 + offsets[0], this.labelY = bbox.y + bbox.height / 2 + offsets[1], this.label = config.canvas.addText({
            text: text,
            "text-anchor": "middle",
    ...
    bindElementEvents: function () {
        ...
        this.container.mousemove(function (e) {
            ...
            var case_under_fake_3 = (code === 'DF4');
            var case_under_fake_4 = (code === 'DF2');
            var case_no_tip = ($('.full-content-explore-list').length == 0 && (case_under_fake_1 || case_under_fake_2 || case_under_fake_3 || case_under_fake_4));
            if ("mouseover" == e.type) {
                (map.container.trigger(overEvent, [code]), overEvent.isDefaultPrevented() || element.setHovered(!0), map.tip.text(tipText), map.container.trigger(tipShowEvent, [map.tip, code]), tipShowEvent.isDefaultPrevented() || ((!case_no_tip ? map.tip.show() : false), map.tipWidth = map.tip.width(), map.tipHeight = map.tip.height()));
                if (!element.isSelected && map.params.regionsSelectable) {
                    var fakeBackgroundColor = map.params.backgroundColor;
                    if (case_under_fake_1) {
                        jQuery('.under_fake_1').attr('fill', fakeBackgroundColor);
                    } else if (case_under_fake_2) {
                        jQuery('.under_fake_2').attr('fill', fakeBackgroundColor);
                ...
                if (!element.isSelected && map.params.regionsSelectable) {
                    var fakeBackgroundColor = "#4F6E99";
                    if (case_under_fake_1) {
                        jQuery('.under_fake_1').attr('fill', fakeBackgroundColor);
                    } else if (case_under_fake_2) {
                        jQuery('.under_fake_2').attr('fill', fakeBackgroundColor);
    ...
    setSelected: function (type, keys) {
        ...
        if ("object" != typeof keys && (keys = [keys]), jvm.$.isArray(keys))
            for (i = 0; i < keys.length; i++) {
                if (typeof this[type][keys[i]] != 'undefined') {
    ...
    getRegionName: function (code) {
        if (typeof this.mapData.paths[code] != "undefined") {
